...
<pre_implementation>
**Write and verify failing tests:**
...
4. Output: files created/modified, coverage summary (happy path, edge cases, errors), confirmation tests fail as expected
...
3. Capture and analyze results: pass/fail counts, error messages and stack traces (first ~50 lines if failures), coverage metrics if available
4. Output:
...
<worktree_testing>
`bazel` only works from main checkout, not inside worktrees. Detect context before running:
```bash
