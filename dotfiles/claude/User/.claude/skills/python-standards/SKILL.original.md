...
All Python code in this repository must adhere to these specific architectural and stylistic standards.

## Core Directives
- **Imports:** Every file MUST start with `from __future__ import annotations`.
- **Logging:** Use `structlog` exclusively. NEVER use `print()` or the standard `logging` library for application logic.
- **Exports:** Every `__init__.py` must define an explicit `__all__` list.

## Type System & Data Modeling
- **Interfaces:** Prefer `typing.Protocol` over `abc.ABC` for structural subtyping.
- **Models:** Use **Pydantic v2** for system boundaries (APIs, Configs). 
  - Always include `model_config = ConfigDict(use_attribute_docstrings=True)`.
...
```
## Execution 
- Always use `uv run` to execute scripts or tools.
